...
// 检索长期保存库档案
const searchLongStoreArchive = () => {
  loading.value = true
  query.archiveTypeId = query.archiveTypeId ? query.archiveTypeId.id : null
  query.formatTypeId = query.formatTypeId ? query.formatTypeId.id : null
  query.archiveDateFrom =
    archiveDate.value === null ? null : archiveDate.value[0]
  query.archiveDateTo = archiveDate.value === null ? null : archiveDate.value[1]
  archiveManage.searchLongStoreArchive(query).then((res: any) => {
    if (res.success) {
...
const handleNodeClick = (data: Tree) => {
  ...
  query.archiveDateFrom = null
  query.archiveDateTo = null
  archiveTypeList.value = data.archiveTypes || []
  searchLongStoreArchive()
}
// 多选操作
