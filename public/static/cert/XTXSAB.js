...
/* globals var */
let $_$softCertListID = '' // Soft CertListID, Set by SetUserCertList
let $_$hardCertListID = '' // USBKeyCertListID, Set by SetUserCertList
let $_$allCertListID = '' // All CertListID, Set by SetUserCertList
let $_$loginCertID = '' // logined CertID, Set by SetAutoLogoutParameter
let $_$logoutFunc = null // logout Function, Set by SetAutoLogoutParameter
let $_$onUsbKeyChangeCallBackFunc = null // custom onUsbkeyChange callback function
let $_$XTXAlert = null // alert custom function
const $_$XTXAppObj = null // XTXAppCOM class Object
const $_$SecXV2Obj = null // BJCASecCOMV2 class Object
const $_$SecXObj = null // BJCASecCOM class Object
const $_$WebSocketObj = null // WebSocket class Object
const $_$CurrentObj = null // Current use class Object
const $_$GetPicObj = null // GetKeyPic class Object

// const var
const CERT_TYPE_HARD = 1
const CERT_TYPE_SOFT = 2
const CERT_TYPE_ALL = 3

// const var
const CERT_OID_VERSION = 1
const CERT_OID_SERIAL = 2
const CERT_OID_SIGN_METHOD = 3
const CERT_OID_ISSUER_C = 4
const CERT_OID_ISSUER_O = 5
const CERT_OID_ISSUER_OU = 6
const CERT_OID_ISSUER_ST = 7
const CERT_OID_ISSUER_CN = 8
const CERT_OID_ISSUER_L = 9
const CERT_OID_ISSUER_E = 10
const CERT_OID_NOT_BEFORE = 11
const CERT_OID_NOT_AFTER = 12
const CERT_OID_SUBJECT_C = 13
const CERT_OID_SUBJECT_O = 14
const CERT_OID_SUBJECT_OU = 15
const CERT_OID_SUBJECT_ST = 16
const CERT_OID_SUBJECT_CN = 17
const CERT_OID_SUBJECT_L = 18
const CERT_OID_SUBJECT_E = 19
const CERT_OID_PUBKEY = 20
const CERT_OID_SUBJECT_DN = 33
const CERT_OID_ISSUER_DN = 34
...
function GetDateNotBefore(strCertValid) {
  const strYear = strCertValid.substring(0, 4)
  const strMonth = strCertValid.substring(4, 6)
  const strDay = strCertValid.substring(6, 8)
  const strHour = strCertValid.substring(8, 10)
  const strMin = strCertValid.substring(10, 12)
  const strSecond = strCertValid.substring(12, 14)
  const RtnDate = new Date()
...
function $popDropListBoxAll(strListID) {
  let objListID = eval(strListID)
  if (objListID == undefined) {
    return
  }
  let i
  const n = objListID.length
  for (i = 0; i < n; i++) {
    objListID.remove(0)
  }
...
function $pushOneDropListBox(userListArray, strListID) {
  ...
  for (i = 0; i < userListArray.length; i++) {
    const certObj = userListArray[i]
    const objItem = new Option(certObj.certName, certObj.certID)
...
function $pushAllDropListBox(certUserListObj) {
  ...
  let strUserList = certUserListObj.retVal
  const allListArray = []
  while (true) {
    const i = strUserList.indexOf('&&&')
    if (i <= 0) {
      break
    }
    const strOneUser = strUserList.substring(0, i)
    const strName = strOneUser.substring(0, strOneUser.indexOf('||'))
    const strCertID = strOneUser.substring(strOneUser.indexOf('||') + 2, strOneUser.length)
...
function $LoadControl(CLSID, ctlName, testFuncName, addEvent) {
  let pluginDiv = document.getElementById(`pluginDiv${ctlName}`)
  if (pluginDiv) {
    return true
  }
  ...
  try {
    if ($checkBrowserISIE()) {
      ...
      const chromeVersion = window.navigator.userAgent.match(/Chrome\/(\d+)\./)
    ...
    //   var a =  eval('XTXAPP.SOF_GetVersion()') ;

    const aaa = eval(`${ctlName}.${testFuncName}`)

    if (
      testFuncName != null &&
      testFuncName != '' &&
      eval(`${ctlName}.${testFuncName}`) == undefined
    ) {
      document.body.removeChild(pluginDiv)
      pluginDiv.innerHTML = ''
      pluginDiv = null
...
function $loginSignRandomCallBack(retObj) {
  ...
  const { objForm } = retObj.ctx
  const strAction = retObj.ctx.action
...
function $loginVerifyServerSignatureCallBack(retObj) {
  ...
  const strCertID = retObj.ctx.certID
  SignedData(strCertID, strServerRan, $loginSignRandomCallBack, retObj.ctx)
}
function $loginCheckCertValidNotAfter(retObj) {
  const notAfterDate = GetDateNotAfter(retObj.retVal)
  const milliseconds = notAfterDate.getTime() - new Date().getTime()
  if (milliseconds < 0) {
  ...
  VerifySignedData(
    ...
    strServerRan,
    strServerSignedData,
    $loginVerifyServerSignatureCallBack,
    retObj.ctx
  )
}

function $loginCheckCertValidNotBefore(retObj) {
  const notBeforeDate = GetDateNotBefore(retObj.retVal)
  const days = parseInt((notBeforeDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
  if (days > 0) {
    $XTXAlert(`您的证书尚未生效!距离生效日期还剩${days}天!`)
    return
  }

  const strUserCert = retObj.ctx.objForm.UserCert.value
...
function $loginGetSignCertCallBack(retObj) {
  const strUserCert = retObj.retVal
  if (strUserCert == '') {
    $XTXAlert('导出用户证书失败!')
    return
  }
...
function $loginGetPINRetryCallBack(retObj) {
  const retryCount = Number(retObj.retVal)
  if (retryCount > 0) {
...
function Login(formName, strCertID, strPin, strAction) {
  ...
  // Add a hidden item ...
  const strSignItem = '<input type="hidden" name="UserSignedData" value="">'
  if (objForm.UserSignedData == null) {
    objForm.insertAdjacentHTML('BeforeEnd', strSignItem)
  }
  const strCertItem = '<input type="hidden" name="UserCert" value="">'
  if (objForm.UserCert == null) {
    objForm.insertAdjacentHTML('BeforeEnd', strCertItem)
  }
  const strContainerItem = '<input type="hidden" name="ContainerName" value="">'
  if (objForm.ContainerName == null) {
    objForm.insertAdjacentHTML('BeforeEnd', strContainerItem)
  }

  const ctx = { certID: strCertID, objForm, action: strAction }
...
function CreateGetPicObject() {
  const bOK = $LoadControl('3BC3C868-95B5-47ED-8686-E0E3E94EF366', 'OGetPic', 'GetPic()', false)
  if (!bOK) {
    return null
  }
}
...
function CreateDsvsObject() {
  // var bOK = $LoadControl("DA4B155C-F991-46C8-896D-2305B364CAA7", "DSVSAPP", "GenRandom()", false);
  const bOK = $LoadControl(
    'DA4B155C-F991-46C8-896D-2305B364CAA7',
    'SVS',
...
function CreateWebSocketObject(myonopen, myonerror) {
  ...
  o.load_websocket = function () {
    let ws_url = 'wss://127.0.0.1:21061/xtxapp/'
    ...
    o.ws_obj.onmessage = function (evt) {
      ...
      const { call_cmd_id } = res
      if (!call_cmd_id) {
        return
      }

      const execFunc = o.ws_queue_list[call_cmd_id]
      if (typeof execFunc !== 'function') {
        return
      }

      let ctx = o.ws_queue_ctx[res.call_cmd_id]
...
// 初始化
window.initCertFunc = function (sunccess, error) {
  // CreateGetPicObject();
  CreateDsvsObject()
  const b = CreateXTXAppObject()
  if (b) {
...
ComInterface.SOF_GetUserList = function (cb, ctx) {
  const ret = XTXAPP.SOF_GetUserList()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_GetCertInfoByOid = function (Cert, Oid, cb, ctx) {
  const ret = XTXAPP.SOF_GetCertInfoByOid(Cert, Oid)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_GetCertInfoByOid = function (Cert, Oid, cb, ctx) {
  const paramArray = [Cert, Oid]
...
ComInterface.SOF_SignData = function (CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_SignData(CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SignData = function (CertID, InData, cb, ctx) {
  const paramArray = [CertID, InData]
...
ComInterface.SOF_VerifySignedData = function (Cert, InData, SignValue, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedData(Cert, InData, SignValue)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedData = function (Cert, InData, SignValue, cb, ctx) {
  const paramArray = [Cert, InData, SignValue]
...
ComInterface.SOF_SignFile = function (CertID, InFile, cb, ctx) {
  const ret = XTXAPP.SOF_SignFile(CertID, InFile)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SignFile = function (CertID, InFile, cb, ctx) {
  const paramArray = [CertID, InFile]
...
ComInterface.SOF_VerifySignedFile = function (Cert, InFile, SignValue, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedFile(Cert, InFile, SignValue)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedFile = function (Cert, InFile, SignValue, cb, ctx) {
  const paramArray = [Cert, InFile, SignValue]
...
ComInterface.SOF_EncryptData = function (Cert, InData, cb, ctx) {
  const ret = XTXAPP.SOF_EncryptData(Cert, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_EncryptData = function (Cert, InData, cb, ctx) {
  const paramArray = [Cert, InData]
...
ComInterface.SOF_DecryptData = function (CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_DecryptData(CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_EncryptFile = function (Cert, InFile, OutFile, cb, ctx) {
  const ret = XTXAPP.SOF_EncryptFile(Cert, InFile, OutFile)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_EncryptFile = function (Cert, InFile, OutFile, cb, ctx) {
  const paramArray = [Cert, InFile, OutFile]
...
ComInterface.SOF_DecryptFile = function (CertID, InFile, OutFile, cb, ctx) {
  const ret = XTXAPP.SOF_DecryptFile(CertID, InFile, OutFile)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_DecryptFile = function (CertID, InFile, OutFile, cb, ctx) {
  const paramArray = [CertID, InFile, OutFile]
...
ComInterface.SOF_SignMessage = function (dwFlag, CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_SignMessage(dwFlag, CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SignMessage = function (dwFlag, CertID, InData, cb, ctx) {
  const paramArray = [dwFlag, CertID, InData]
...
ComInterface.SOF_VerifySignedMessage = function (MessageData, InData, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedMessage(MessageData, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedMessage = function (MessageData, InData, cb, ctx) {
  const paramArray = [MessageData, InData]
...
ComInterface.SOF_GetInfoFromSignedMessage = function (SignedMessage, type, cb, ctx) {
  const ret = XTXAPP.SOF_GetInfoFromSignedMessage(SignedMessage, type)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_GetInfoFromSignedMessage = function (SignedMessage, type, cb, ctx) {
  const paramArray = [SignedMessage, type]
...
ComInterface.SOF_SignDataXML = function (CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_SignDataXML(CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_VerifySignedDataXML = function (InData, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedDataXML(InData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedDataXML = function (InData, cb, ctx) {
  const paramArray = [InData]
...
ComInterface.SOF_GetXMLSignatureInfo = function (XMLSignedData, type, cb, ctx) {
  const ret = XTXAPP.SOF_GetXMLSignatureInfo(XMLSignedData, type)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_GetXMLSignatureInfo = function (XMLSignedData, type, cb, ctx) {
  const paramArray = [XMLSignedData, type]
...
ComInterface.SOF_PriKeyDecrypt = function (CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_PriKeyDecrypt(CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.GetDeviceCount = function (cb, ctx) {
  const ret = XTXAPP.GetDeviceCount()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.GetAllDeviceSN = function (cb, ctx) {
  const ret = XTXAPP.GetAllDeviceSN()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.GetDeviceSNByIndex = function (iIndex, cb, ctx) {
  const ret = XTXAPP.GetDeviceSNByIndex(iIndex)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.GetDeviceSNByIndex = function (iIndex, cb, ctx) {
  const paramArray = [iIndex]
...
ComInterface.GetDeviceInfo = function (sDeviceSN, iType, cb, ctx) {
  const ret = XTXAPP.GetDeviceInfo(sDeviceSN, iType)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.GetDeviceInfo = function (sDeviceSN, iType, cb, ctx) {
  const paramArray = [sDeviceSN, iType]
...
ComInterface.ChangeAdminPass = function (sDeviceSN, sOldPass, sNewPass, cb, ctx) {
  const ret = XTXAPP.ChangeAdminPass(sDeviceSN, sOldPass, sNewPass)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ChangeAdminPass = function (sDeviceSN, sOldPass, sNewPass, cb, ctx) {
  const paramArray = [sDeviceSN, sOldPass, sNewPass]
...
ComInterface.UnlockUserPass = function (sDeviceSN, sAdminPass, sNewUserPass, cb, ctx) {
  const ret = XTXAPP.UnlockUserPass(sDeviceSN, sAdminPass, sNewUserPass)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.UnlockUserPass = function (sDeviceSN, sAdminPass, sNewUserPass, cb, ctx) {
  const paramArray = [sDeviceSN, sAdminPass, sNewUserPass]
...
ComInterface.GenerateKeyPair = function (sDeviceSN, sContainerName, iKeyType, bSign, cb, ctx) {
  const ret = XTXAPP.GenerateKeyPair(sDeviceSN, sContainerName, iKeyType, bSign)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.GenerateKeyPair = function (
  sDeviceSN,
  sContainerName,
  iKeyType,
  bSign,
  ...
  const paramArray = [sDeviceSN, sContainerName, iKeyType, bSign]
...
ComInterface.ExportPubKey = function (sDeviceSN, sContainerName, bSign, cb, ctx) {
  const ret = XTXAPP.ExportPubKey(sDeviceSN, sContainerName, bSign)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ExportPubKey = function (sDeviceSN, sContainerName, bSign, cb, ctx) {
  const paramArray = [sDeviceSN, sContainerName, bSign]
...
ComInterface.ImportSignCert = function (sDeviceSN, sContainerName, sCert, cb, ctx) {
  const ret = XTXAPP.ImportSignCert(sDeviceSN, sContainerName, sCert)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ImportSignCert = function (sDeviceSN, sContainerName, sCert, cb, ctx) {
  const paramArray = [sDeviceSN, sContainerName, sCert]
...
ComInterface.ImportEncCert = function (sDeviceSN, sContainerName, sCert, sPriKeyCipher, cb, ctx) {
  const ret = XTXAPP.ImportEncCert(sDeviceSN, sContainerName, sCert, sPriKeyCipher)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ImportEncCert = function (
  sDeviceSN,
  sContainerName,
  sCert,
  sPriKeyCipher,
...
ComInterface.WriteFile = function (sDeviceSN, sFileName, sContent, bPrivate, cb, ctx) {
  const ret = XTXAPP.WriteFile(sDeviceSN, sFileName, sContent, bPrivate)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.WriteFile = function (sDeviceSN, sFileName, sContent, bPrivate, cb, ctx) {
  const paramArray = [sDeviceSN, sFileName, sContent, bPrivate]
...
ComInterface.IsContainerExist = function (sDeviceSN, sContainerName, cb, ctx) {
  const ret = XTXAPP.IsContainerExist(sDeviceSN, sContainerName)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.IsContainerExist = function (sDeviceSN, sContainerName, cb, ctx) {
  const paramArray = [sDeviceSN, sContainerName]
...
ComInterface.DeleteContainer = function (sDeviceSN, sContainerName, cb, ctx) {
  const ret = XTXAPP.DeleteContainer(sDeviceSN, sContainerName)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ExportPKCS10 = function (sDeviceSN, sContainerName, sDN, bSign, cb, ctx) {
  const ret = XTXAPP.ExportPKCS10(sDeviceSN, sContainerName, sDN, bSign)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ExportPKCS10 = function (sDeviceSN, sContainerName, sDN, bSign, cb, ctx) {
  const paramArray = [sDeviceSN, sContainerName, sDN, bSign]
...
ComInterface.AlertBeforeCertDate = function (newVal, cb, ctx) {
  const ret = XTXAPP.AlertBeforeCertDate(newVal)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ServerSignedData = function (cb, ctx) {
  const ret = XTXAPP.ServerSignedData()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ServerSignedData = function (newVal, cb, ctx) {
  const ret = XTXAPP.ServerSignedData(newVal)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ServerRan = function (newVal, cb, ctx) {
  const ret = XTXAPP.ServerRan(newVal)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ServerCert = function (newVal, cb, ctx) {
  const ret = XTXAPP.ServerCert(newVal)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ServerMode = function (newVal, cb, ctx) {
  const ret = XTXAPP.ServerMode(newVal)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ShowError = function (newVal, cb, ctx) {
  const ret = XTXAPP.ShowError(newVal)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_ExportExChangeUserCert = function (CertID, cb, ctx) {
  const ret = XTXAPP.SOF_ExportExChangeUserCert(CertID)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.GetContainerCount = function (sDeviceSN, cb, ctx) {
  const ret = XTXAPP.GetContainerCount(sDeviceSN)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_SymEncryptData = function (sKey, indata, cb, ctx) {
  const ret = XTXAPP.SOF_SymEncryptData(sKey, indata)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SymEncryptData = function (sKey, indata, cb, ctx) {
  const paramArray = [sKey, indata]
...
ComInterface.SOF_SymDecryptData = function (sKey, indata, cb, ctx) {
  const ret = XTXAPP.SOF_SymDecryptData(sKey, indata)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_SymEncryptFile = function (sKey, inFile, outFile, cb, ctx) {
  const ret = XTXAPP.SOF_SymEncryptFile(sKey, inFile, outFile)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SymEncryptFile = function (sKey, inFile, outFile, cb, ctx) {
  const paramArray = [sKey, inFile, outFile]
...
ComInterface.SOF_SymDecryptFile = function (sKey, inFile, outFile, cb, ctx) {
  const ret = XTXAPP.SOF_SymDecryptFile(sKey, inFile, outFile)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_GetLastErrMsg = function (cb, ctx) {
  const ret = XTXAPP.SOF_GetLastErrMsg()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.WriteFileEx = function (sDeviceSN, sFileName, sContent, bPrivate, cb, ctx) {
  const ret = XTXAPP.WriteFileEx(sDeviceSN, sFileName, sContent, bPrivate)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.ReadFileEx = function (sDeviceSN, sFileName, cb, ctx) {
  const ret = XTXAPP.ReadFileEx(sDeviceSN, sFileName)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_EncryptDataEx = function (Cert, InData, cb, ctx) {
  const ret = XTXAPP.SOF_EncryptDataEx(Cert, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_GetAllContainerName = function (sDeviceSN, cb, ctx) {
  const ret = XTXAPP.SOF_GetAllContainerName(sDeviceSN)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.CreateSoftDevice = function (sDeviceSN, sLabel, cb, ctx) {
  const ret = XTXAPP.CreateSoftDevice(sDeviceSN, sLabel)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.CreateSoftDevice = function (sDeviceSN, sLabel, cb, ctx) {
  const paramArray = [sDeviceSN, sLabel]
...
ComInterface.DeleteSoftDevice = function (sDeviceSN, sPasswd, cb, ctx) {
  const ret = XTXAPP.DeleteSoftDevice(sDeviceSN, sPasswd)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.DeleteSoftDevice = function (sDeviceSN, sPasswd, cb, ctx) {
  const paramArray = [sDeviceSN, sPasswd]
...
ComInterface.EnableSoftDevice = function (enable, sDeviceSN, cb, ctx) {
  const ret = XTXAPP.EnableSoftDevice(enable, sDeviceSN)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.EnableSoftDevice = function (enable, sDeviceSN, cb, ctx) {
  const paramArray = [enable, sDeviceSN]
...
ComInterface.SoftDeviceBackup = function (sDeviceSN, sPasswd, cb, ctx) {
  const ret = XTXAPP.SoftDeviceBackup(sDeviceSN, sPasswd)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_VerifySignedByteData = function (Cert, byteLen, SignValue, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedByteData(Cert, byteLen, SignValue)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedByteData = function (Cert, byteLen, SignValue, cb, ctx) {
  const paramArray = [Cert, byteLen, SignValue]
...
ComInterface.ImportEncCertEx = function (
  ...
  const ret = XTXAPP.ImportEncCertEx(sDeviceSN, sContainerName, sCert, sPriKeyCipher, ulSymAlg)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ImportEncCertEx = function (
  sDeviceSN,
  sContainerName,
  sCert,
  sPriKeyCipher,
...
ComInterface.SOF_HMAC = function (hashid, key, indata, cb, ctx) {
  const ret = XTXAPP.SOF_HMAC(hashid, key, indata)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_HMAC = function (hashid, key, indata, cb, ctx) {
  const paramArray = [hashid, key, indata]
...
ComInterface.ImportKeyCertToSoftDevice = function (
  ...
  const ret = XTXAPP.ImportKeyCertToSoftDevice(sDeviceSN, sContainerName, sPriKey, sCert, bSign)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ImportKeyCertToSoftDevice = function (
  sDeviceSN,
  sContainerName,
  sPriKey,
  sCert,
  ...
  const paramArray = [sDeviceSN, sContainerName, sPriKey, sCert, bSign]
...
WebsocketInterface.InitDeviceEx = function (
  ...
  const paramArray = [sDeviceSN, sAdminPass, sUserPin, sKeyLabel, adminPinMaxRetry, userPinMaxRetry]
  WebsocketApp.callMethod('InitDeviceEx', cb, ctx, 'bool', paramArray)
}

ComInterface.SelectFile = function (cb, ctx) {
  const ret = XTXAPP.SelectFile()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_SignHashData = function (CertID, b64ashData, hashAlg, cb, ctx) {
  const ret = XTXAPP.SOF_SignHashData(CertID, b64ashData, hashAlg)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SignHashData = function (CertID, b64ashData, hashAlg, cb, ctx) {
  const paramArray = [CertID, b64ashData, hashAlg]
...
ComInterface.SOF_VerifySignedHashData = function (Cert, b64ashData, SignValue, hashAlg, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedHashData(Cert, b64ashData, SignValue, hashAlg)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedHashData = function (
  Cert,
  b64ashData,
  SignValue,
  hashAlg,
...
ComInterface.SOF_SignBinaryData = function (CertID, cb, ctx) {
  const ret = XTXAPP.SOF_SignBinaryData(CertID)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_VerifySignedBinaryData = function (Cert, SignValue, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedBinaryData(Cert, SignValue)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedBinaryData = function (Cert, SignValue, cb, ctx) {
  const paramArray = [Cert, SignValue]
...
ComInterface.SOF_EncryptBinaryData = function (Cert, cb, ctx) {
  const ret = XTXAPP.SOF_EncryptBinaryData(Cert)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_EncryptBinaryDataEx = function (Cert, cb, ctx) {
  const ret = XTXAPP.SOF_EncryptBinaryDataEx(Cert)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_DecryptBinaryData = function (CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_DecryptBinaryData(CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_SignBinaryMessage = function (dwFlag, CertID, cb, ctx) {
  const ret = XTXAPP.SOF_SignBinaryMessage(dwFlag, CertID)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SignBinaryMessage = function (dwFlag, CertID, cb, ctx) {
  const paramArray = [dwFlag, CertID]
...
ComInterface.SOF_VerifySignedBinaryMessage = function (MessageData, cb, ctx) {
  const ret = XTXAPP.SOF_VerifySignedBinaryMessage(MessageData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_VerifySignedBinaryMessage = function (MessageData, cb, ctx) {
  const paramArray = [MessageData]
...
ComInterface.SOF_PubKeyBinaryEncrypt = function (Cert, cb, ctx) {
  const ret = XTXAPP.SOF_PubKeyBinaryEncrypt(Cert)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_PriKeyBinaryDecrypt = function (CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_PriKeyBinaryDecrypt(CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_Base64BinaryEncode = function (cb, ctx) {
  const ret = XTXAPP.SOF_Base64BinaryEncode()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_Base64BinaryDecode = function (sIndata, cb, ctx) {
  const ret = XTXAPP.SOF_Base64BinaryDecode(sIndata)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_HashBinaryData = function (hashAlg, sCert, sID, cb, ctx) {
  const ret = XTXAPP.SOF_HashBinaryData(hashAlg, sCert, sID)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_HashBinaryData = function (hashAlg, sCert, sID, cb, ctx) {
  const paramArray = [hashAlg, sCert, sID]
...
ComInterface.ImportPfxToDevice = function (
  ...
  const ret = XTXAPP.ImportPfxToDevice(sDeviceSN, sContainerName, bSign, strPfx, strPfxPass)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ImportPfxToDevice = function (
  sDeviceSN,
  sContainerName,
  bSign,
  strPfx,
  ...
  const paramArray = [sDeviceSN, sContainerName, bSign, strPfx, strPfxPass]
...
ComInterface.SOF_HashDataEx = function (hashAlg, sInData, sCert, sID, cb, ctx) {
  const ret = XTXAPP.SOF_HashDataEx(hashAlg, sInData, sCert, sID)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_HashDataEx = function (hashAlg, sInData, sCert, sID, cb, ctx) {
  const paramArray = [hashAlg, sInData, sCert, sID]
...
ComInterface.SOF_HashDataExBase64 = function (hashAlg, sInData, sCert, sID, cb, ctx) {
  const ret = XTXAPP.SOF_HashDataExBase64(hashAlg, sInData, sCert, sID)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.GetAllDeviceSNEx = function (type, cb, ctx) {
  const ret = XTXAPP.GetAllDeviceSNEx(type)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_UpdateCert = function (CertID, type, cb, ctx) {
  const ret = XTXAPP.SOF_UpdateCert(CertID, type)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_UpdateCert = function (CertID, type, cb, ctx) {
  const paramArray = [CertID, type]
...
ComInterface.OpenSpecifiedFolder = function (backupFilePath, cb, ctx) {
  const ret = XTXAPP.OpenSpecifiedFolder(backupFilePath)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.OpenSpecifiedFolder = function (backupFilePath, cb, ctx) {
  const paramArray = [backupFilePath]
...
ComInterface.OTP_GetChallengeCodeEx = function (sCertID, szAccount, money, cb, ctx) {
  const ret = XTXAPP.OTP_GetChallengeCodeEx(sCertID, szAccount, money)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.OTP_GetChallengeCodeEx = function (sCertID, szAccount, money, cb, ctx) {
  const paramArray = [sCertID, szAccount, money]
...
ComInterface.OTP_Halt = function (sCertID, cb, ctx) {
  const ret = XTXAPP.OTP_Halt(sCertID)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_TSGenREQ = function (
  b64Hash,
  hashAlg,
  bReqCert,
  policyID,
  ...
  const ret = XTXAPP.SOF_TSGenREQ(b64Hash, hashAlg, bReqCert, policyID, b64Nonce, b64Extension)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_TSGenREQ = function (
  b64Hash,
  hashAlg,
  bReqCert,
  policyID,
  ...
  const paramArray = [b64Hash, hashAlg, bReqCert, policyID, b64Nonce, b64Extension]
...
ComInterface.SOF_TSCompareNonce = function (b64TSReq, b64TSAResp, cb, ctx) {
  const ret = XTXAPP.SOF_TSCompareNonce(b64TSReq, b64TSAResp)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_TSCompareNonce = function (b64TSReq, b64TSAResp, cb, ctx) {
  const paramArray = [b64TSReq, b64TSAResp]
...
ComInterface.SOF_TSGenPDFSignature = function (b64TSAResp, b64OriPDFSignature, cb, ctx) {
  const ret = XTXAPP.SOF_TSGenPDFSignature(b64TSAResp, b64OriPDFSignature)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_TSGenPDFSignature = function (b64TSAResp, b64OriPDFSignature, cb, ctx) {
  const paramArray = [b64TSAResp, b64OriPDFSignature]
...
ComInterface.SOF_TSVerifyPDFSignature = function (b64TSPDFSignature, cb, ctx) {
  const ret = XTXAPP.SOF_TSVerifyPDFSignature(b64TSPDFSignature)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_TSVerifyPDFSignature = function (b64TSPDFSignature, cb, ctx) {
  const paramArray = [b64TSPDFSignature]
...
ComInterface.SOF_TSGetPDFSignatureInfo = function (b64TSPDFSignature, iType, cb, ctx) {
  const ret = XTXAPP.SOF_TSGetPDFSignatureInfo(b64TSPDFSignature, iType)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_TSGetPDFSignatureInfo = function (b64TSPDFSignature, iType, cb, ctx) {
  const paramArray = [b64TSPDFSignature, iType]
...
ComInterface.OTP_GetState = function (sCertID, bCert, cb, ctx) {
  const ret = XTXAPP.OTP_GetState(sCertID, bCert)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.OTP_GetState = function (sCertID, bCert, cb, ctx) {
  const paramArray = [sCertID, bCert]
...
ComInterface.ExportPfxFromDevice = function (
  sDeviceSN,
  sContainerName,
  bSign,
  strPfxPass,
  ...
  const ret = XTXAPP.ExportPfxFromDevice(sDeviceSN, sContainerName, bSign, strPfxPass)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ExportPfxFromDevice = function (
  sDeviceSN,
  sContainerName,
  bSign,
  strPfxPass,
  ...
  const paramArray = [sDeviceSN, sContainerName, bSign, strPfxPass]
...
ComInterface.SOF_SignHashMessage = function (CertID, InHashData, hashAlg, cb, ctx) {
  const ret = XTXAPP.SOF_SignHashMessage(CertID, InHashData, hashAlg)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SignHashMessage = function (CertID, InHashData, hashAlg, cb, ctx) {
  const paramArray = [CertID, InHashData, hashAlg]
...
ComInterface.ExportPfxToFile = function (sDeviceSN, sContainerName, bSign, strPfxPass, cb, ctx) {
  const ret = XTXAPP.ExportPfxToFile(sDeviceSN, sContainerName, bSign, strPfxPass)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ExportPfxToFile = function (
  sDeviceSN,
  sContainerName,
  bSign,
  strPfxPass,
...
ComInterface.SOF_SignEnvelope = function (CertID, Cert, InData, cb, ctx) {
  const ret = XTXAPP.SOF_SignEnvelope(CertID, Cert, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.SOF_SignEnvelope = function (CertID, Cert, InData, cb, ctx) {
  const paramArray = [CertID, Cert, InData]
...
ComInterface.SOF_UnSignEnvelope = function (CertID, InData, cb, ctx) {
  const ret = XTXAPP.SOF_UnSignEnvelope(CertID, InData)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_GetLastLoginCertID = function (cb, ctx) {
  const ret = XTXAPP.SOF_GetLastLoginCertID()
  return $myOKRtnFunc(ret, cb, ctx)
}
...
ComInterface.SOF_GetLastSignDataCertID = function (CertID, cb, ctx) {
  const ret = XTXAPP.SOF_GetLastSignDataCertID(CertID)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
PicComInterface.GetPic1 = function (bstrConName, cb, ctx) {
  const ret = OGetPic.GetPic1(bstrConName)
  return $myOKRtnFunc(ret, cb, ctx)
}
...
PicComInterface.ConvertPicSize = function (bstrPic, w, h, cb, ctx) {
  const ret = OGetPic.ConvertPicSize(bstrPic, w, h)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.ConvertPicSize = function (bstrPic, w, h, cb, ctx) {
  const paramArray = [bstrPic, w, h]
...
DsvsInterface.GenRandom = function (nRandom, cb, ctx) {
  // var ret = DSVSAPP.GenRandom(nRandom);
  const ret = SVS.GenRandom(nRandom)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.GenRandom = function (nRandom, cb, ctx) {
  const paramArray = [nRandom]
  WebsocketApp.callMethod('GenRandom', cb, ctx, 'string', paramArray)
}

ComInterface.VerifySignedData = function (bstrCert, bstrData, bstrSignedData, cb, ctx) {
  const ret = SVS.VerifySignedData(bstrCert, bstrData, bstrSignedData)
  return $myOKRtnFunc(ret, cb, ctx)
}

WebsocketInterface.VerifySignedData = function (bstrCert, bstrData, bstrSignedData, cb, ctx) {
  const paramArray = [bstrCert, bstrData, bstrSignedData]
  WebsocketApp.callMethod('VerifySignedData', cb, ctx, 'int', paramArray)
}

init_Interface('GenRandom')
init_Interface('VerifySignedData')
// dsvs interface end
